...
const MultiPrintModal = ({ show, onHide, items }) => {
    ...
    return (
        <Modal show={show} onHide={onHide} size="lg" centered className="item-detail-modal">
            <Modal.Header closeButton className="d-print-none border-0">
                <Modal.Title className="fw-bold">รายละเอียดพัสดุ ({items.length} รายการ)</Modal.Title>
            </Modal.Header>
            <Modal.Body className="p-0">
                <div ref={printRef} className="multi-document-container">
                    {items.map((item, index) => (
                        <div key={item.id || index} className="document-container bg-white mx-auto shadow-sm position-relative"
                            style={{ 
                                width: '101.6mm', 
                                minHeight: '72.4mm', 
                                padding: '4mm 5mm', 
            ...
            <style>{`
                .item-detail-modal .modal-content {
                    border: none;
                    border-radius: 12px;
                    overflow: auto; /* Allow scrolling if many items */
                    max-height: 90vh;
                }
                .document-container {
